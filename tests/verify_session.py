...
def simulate_session():
    ...
    for i in range(3):
        telemetry = {'switch_rate': 0.1, 'motor_var': 0.05, 'distractor_attempts': 0, 'idle_ratio': 0.05, 'task_eng': 1.0, 'is_idle': 0}
        ...
        state = {'I_t': engine.I_t, 'D_t': engine.D_t, 'F_t': engine.F_t, 'ECN_t': engine.ECN_t, 'A_t': engine.A_t}
        x_vec = predictor.construct_feature_vector(state)
    ...
    for i in range(3, 8):
        telemetry = {'switch_rate': 1.5, 'motor_var': 1.2, 'distractor_attempts': 3, 'idle_ratio': 0.1, 'task_eng': 0.5, 'is_idle': 0}
        engine.update_latent_states(telemetry, i * 5)
        ...
        if prob > 0.5:
            # Output explanation for high risk
            explanation = predictor.explain_prediction(x_vec)
            top_feature = list(explanation.keys())[0]
            print(f"  [EXPLAIN]: Primary risk factor is {top_feature}")
            
        # User feedback: Yes, I am distracted (Correct)
